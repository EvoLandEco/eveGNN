...
#' @export plot_scatter_difference_by_pars
plot_scatter_difference_by_pars <- function(path=NULL, task="BD", model="diffpool", max_depth=5, which = "nnode", abline_color = "red", abline_range = c(-100, 100)) {
  xvar <- NULL
  if (which == "nnode") {
    xvar <- "num_nodes"
  ...
  if (task != "EVE") {
    if (task == "DDD") {
      ...
      plot_data_mle <- plot_data_mle %>% dplyr::filter(lambda_r_diff < 2000, lambda_r_diff > -2000, mu_r_diff < 2000, mu_r_diff > -2000, cap_r_diff < 2000, cap_r_diff > -2000) %>% tidyr::gather("Parameter", "Value", -Task, -Model, -num_nodes, -lambda, -mu, -cap)
    } else if (task == "BD") {
      plot_data_mle <- load_full_mle_result(path = path, task_type = paste0(task,"_MLE_TES"), model_type = model)
      plot_data_mle <- plot_data_mle %>% dplyr::filter(lambda_r_diff < 2000, lambda_r_diff > -2000, mu_r_diff < 2000, mu_r_diff > -2000) %>% tidyr::gather("Parameter", "Value", -Task, -Model, -num_nodes, -lambda, -mu)
    } else if (task == "PBD") {
  ...
  if (task == "BD") {
    plot_data <- plot_data %>% dplyr::select(-lambda_diff, -mu_diff, -lambda_pred, -mu_pred) %>%
    ...
    plot_data <- plot_data %>% dplyr::select(-lambda_diff, -mu_diff, -lambda_pred, -mu_pred, -cap_diff, -cap_pred) %>%
      dplyr::filter(lambda_r_diff < 250, mu_r_diff < 250, lambda_r_diff > -250, mu_r_diff > -250) %>%
...
plot_boxplot_difference <- function(path=NULL, task="BD", model="diffpool", max_depth=5) {
  ...
  if (task == "BD") {
    plot_data <- plot_data %>% dplyr::select(lambda_r_diff, mu_r_diff, Depth, Model, Task) %>%
      dplyr::filter(lambda_r_diff < 250, mu_r_diff < 250, lambda_r_diff > -250, mu_r_diff > -250) %>%
      tidyr::gather("Parameter", "Value", -Task, -Model, -Depth)
  } else if (task == "DDD") {
    plot_data <- plot_data %>% dplyr::select(lambda_r_diff, mu_r_diff, cap_r_diff, Depth, Model, Task) %>%
      dplyr::filter(lambda_r_diff < 250, mu_r_diff < 250, lambda_r_diff > -250, mu_r_diff > -250) %>%
      tidyr::gather("Parameter", "Value", -Task, -Model, -Depth)
  }
  ...
  plot <- ggplot2::ggplot(data = plot_data_all) +
    ...
    ggplot2::facet_wrap(.~Parameter, scales = "free",
                        labeller = ggplot2::labeller(Parameter = ggplot2::as_labeller(~difference_var_to_label(.x), ggplot2::label_parsed))) +
    ggplot2::labs(x = NULL, y = "Relative difference", fill = "Number of GNN layer(s)") +
...
#' @export plot_scatter_difference_in_out_sample
plot_scatter_difference_in_out_sample <- function(path=NULL, task="BD", model="diffpool", max_depth=5, which = "lambda", abline_color = "red", abline_range = c(-100, 100)) {
  xvar <- NULL
  if (which == "lambda") {
    xvar <- "lambda"
    xlabel <- "Speciation rate"
  ...
  plot_data <- plot_data %>% dplyr::select(dplyr::all_of(data_var_list)) %>%
