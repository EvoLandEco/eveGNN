...
plot_model_performance <- function(data, group_var = stop("Grouping variable not provided."), legend_title = NULL, accuracy_ref = NULL, loss_ref = NULL) {
  ...
  accu_plot <- ggplot2::ggplot(data) +
    ggplot2::geom_line(ggplot2::aes(x = Epoch, y = Test_Accuracy_Overall, color = eval(parse(text = group_var)))) +
  ...
  class0_plot <- ggplot2::ggplot(data) +
    ggplot2::geom_line(ggplot2::aes(x = Epoch, y = Test_Class0_Accuracy, color = eval(parse(text = group_var)))) +
    ggplot2::geom_hline(yintercept = accuracy_ref, linetype = "dashed", color = "black") +
    #ggplot2::ylim(0, loss_ref) +
    ggplot2::annotate("text",
                      x = median(data$Epoch),
    ...
    #                    hjust = 0, nudge_x = 0.1) +
    ggplot2::guides(color = ggplot2::guide_legend(nrow = 2)) +
    ggplot2::labs(y = "Class 0 Accuracy", color = legend_title) +
    ggplot2::theme(aspect.ratio = 5 / 6)

  class1_plot <- ggplot2::ggplot(data) +
    ggplot2::geom_line(ggplot2::aes(x = Epoch, y = Test_Class1_Accuracy, color = eval(parse(text = group_var)))) +
    ggplot2::geom_hline(yintercept = accuracy_ref, linetype = "dashed", color = "black") +
    #ggplot2::ylim(0, loss_ref) +
    ggplot2::annotate("text",
                      x = median(data$Epoch),
    ...
    # Add labels at the end of the line
    # ggplot2::geom_text(data = dplyr::filter(data, Epoch == max(Epoch)),
    #                    ggplot2::aes(x = Epoch, y = Test_Class1_Accuracy, label = cap),
    #                    hjust = 0, nudge_x = 0.1) +
    ggplot2::guides(color = ggplot2::guide_legend(nrow = 2)) +
    ggplot2::labs(y = "Class 1 Accuracy", color = legend_title) +
    ggplot2::theme(aspect.ratio = 5 / 6)
