...
submit_job() {
  local name=$1
  local set_i=$2
  local task_type=$3  # This will be either DDD_TAS or EVE_TAS
...
ddd_path="$name/DDD_TAS"
eve_path="$name/EVE_TAS"

# Iterate through the sets in DDD_TAS and submit jobs
for set_dir in "$ddd_path"/set_*; do
    set_i=$(basename "$set_dir")
    submit_job "$name" "$set_i" "DDD_TAS"
done

# Iterate through the sets in EVE_TAS and submit jobs
for set_dir in "$eve_path"/set_*; do
    set_i=$(basename "$set_dir")
    submit_job "$name" "$set_i" "EVE_TAS"
done
