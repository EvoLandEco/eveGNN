...
def main():
    ...
    current_dataset = current_dataset + current_val_dataset
    ...
    # Save the model
    print(f"Saving model to {os.path.join(name, task_type, f'{task_type}_model_diffpool_full_{gnn_depth}.pt')}")
    torch.save(model.state_dict(), os.path.join(name, task_type, f"{task_type}_model_diffpool_full_{gnn_depth}.pt"))
    ...
    pyreadr.write_rds(os.path.join(name, task_type, f"{task_type}_diffpool_full_{gnn_depth}.rds"), model_performance)
    pyreadr.write_rds(os.path.join(name, task_type, f"{task_type}_final_diffs_diffpool_full_{gnn_depth}.rds"), final_differences)
    pyreadr.write_rds(os.path.join(name, task_type, f"{task_type}_final_predictions_diffpool_full_{gnn_depth}.rds"), final_predictions)
    pyreadr.write_rds(os.path.join(name, task_type, f"{task_type}_final_y_diffpool_full_{gnn_depth}.rds"), final_y)
