...
data <- readRDS(file.path(name, "DDD_MLE_TES/MLE_DATA/ddd_mle_poly.rds"))

setwd(name)

if (!dir.exists("DDD_MLE_POLY")) {
  dir.create("DDD_MLE_POLY")
}
...
tryCatch(
  R.utils::withTimeout({
    ml <- DDD::dd_ML(
      brts = data$brts[[i]],
      initparsopt = c(runif(1, 1, 3), runif(1, 0, 1), runif(1, 200, 800)),
      idparsopt = c(1, 2, 3),
      btorph = 0,
      soc = 2,
      cond = 1,
