...
def main():
    ...
    filtered_validation_data = [data for data in filtered_validation_data if
                                data.edge_index.shape != torch.Size([2, 1])]
    ...
    for epoch in range(1, epoch_number_gnn):
        ...
        print(
            f'Epoch: {epoch:03d}, Par 1 Mean Diff: {test_mean_diffs[0]:.4f}, Par 2 Mean Diff: {test_mean_diffs[1]:.4f}, Par 3 Mean Diff: {test_mean_diffs[2]:.4f}, Par 4 Mean Diff: {test_mean_diffs[3]:.4f}, Par 5 Mean Diff: {test_mean_diffs[4]:.4f}, Train Loss: {train_loss_all:.4f}, Test Loss: {test_loss_all:.4f}')
    ...
    final_differences = pd.DataFrame(final_test_diffs,
                                     columns=["lambda1_diff", "lambda2_diff", "lambda3_diff", "mu1_diff", "mu2_diff"])
    final_predictions = pd.DataFrame(final_test_predictions,
                                     columns=["lambda1_pred", "lambda2_pred", "lambda3_pred", "mu1_pred", "mu2_pred"])
    ...
    print(abs(final_differences[["lambda1_diff", "lambda2_diff", "lambda3_diff", "mu1_diff", "mu2_diff"]]).mean())
    ...
    dnn_compensation_data_dict = {"res_lambda1_before": residuals_before_dnn[:, 0],
                                  "res_lambda2_before": residuals_before_dnn[:, 1],
                                  "res_lambda3_before": residuals_before_dnn[:, 2],
                                  "res_mu1_before": residuals_before_dnn[:, 3],
                                  "res_mu2_before": residuals_before_dnn[:, 4],
                                  "res_lambda1_after": residuals_after_dnn[:, 0],
                                  "res_lambda2_after": residuals_after_dnn[:, 1],
                                  "res_lambda3_after": residuals_after_dnn[:, 2],
                                  "res_mu1_after": residuals_after_dnn[:, 3],
                                  "res_mu2_after": residuals_after_dnn[:, 4],
                                  "pred_res_lambda1": predicted_residuals_dnn[:, 0],
                                  "pred_res_lambda2": predicted_residuals_dnn[:, 1],
                                  "pred_res_lambda3": predicted_residuals_dnn[:, 2],
                                  "pred_res_mu1": predicted_residuals_dnn[:, 3],
                                  "pred_res_mu2": predicted_residuals_dnn[:, 4],
                                  "pred_lambda1_before": predictions_before_dnn[:, 0],
                                  "pred_lambda2_before": predictions_before_dnn[:, 1],
                                  "pred_lambda3_before": predictions_before_dnn[:, 2],
                                  "pred_mu1_before": predictions_before_dnn[:, 3],
                                  "pred_mu2_before": predictions_before_dnn[:, 4],
                                  "pred_lambda1_after": predictions_after_dnn[:, 0],
                                  "pred_lambda2_after": predictions_after_dnn[:, 1],
                                  "pred_lambda3_after": predictions_after_dnn[:, 2],
                                  "pred_mu1_after": predictions_after_dnn[:, 3],
                                  "pred_mu2_after": predictions_after_dnn[:, 4],
                                  "lambda1": y_original[:, 0],
                                  "lambda2": y_original[:, 1],
                                  "lambda3": y_original[:, 2],
                                  "mu1": y_original[:, 3],
                                  "mu2": y_original[:, 4],
    ...
    lstm_compensation_data_dict = {"res_lambda1_before": residuals_before_lstm[:, 0],
                                   "res_lambda2_before": residuals_before_lstm[:, 1],
                                   "res_lambda3_before": residuals_before_lstm[:, 2],
                                   "res_mu1_before": residuals_before_lstm[:, 3],
                                   "res_mu2_before": residuals_before_lstm[:, 4],
                                   "res_lambda1_after": residuals_after_lstm[:, 0],
                                   "res_lambda2_after": residuals_after_lstm[:, 1],
                                   "res_lambda3_after": residuals_after_lstm[:, 2],
                                   "res_mu1_after": residuals_after_lstm[:, 3],
                                   "res_mu2_after": residuals_after_lstm[:, 4],
                                   "pred_res_lambda1": predicted_residuals_lstm[:, 0],
                                   "pred_res_lambda2": predicted_residuals_lstm[:, 1],
                                   "pred_res_lambda3": predicted_residuals_lstm[:, 2],
                                   "pred_res_mu1": predicted_residuals_lstm[:, 3],
                                   "pred_res_mu2": predicted_residuals_lstm[:, 4],
                                   "pred_lambda1_before": predictions_before_lstm[:, 0],
                                   "pred_lambda2_before": predictions_before_lstm[:, 1],
                                   "pred_lambda3_before": predictions_before_lstm[:, 2],
                                   "pred_mu1_before": predictions_before_lstm[:, 3],
                                   "pred_mu2_before": predictions_before_lstm[:, 4],
                                   "pred_lambda1_after": predictions_after_lstm[:, 0],
                                   "pred_lambda2_after": predictions_after_lstm[:, 1],
                                   "pred_lambda3_after": predictions_after_lstm[:, 2],
                                   "pred_mu1_after": predictions_after_lstm[:, 3],
                                   "pred_mu2_after": predictions_after_lstm[:, 4],
                                   "lambda1": y_original[:, 0],
                                   "lambda2": y_original[:, 1],
                                   "lambda3": y_original[:, 2],
                                   "mu1": y_original[:, 3],
    ...
    residuals_train_loader_for_lstm_dnn = DataLoader(residuals_training_dataset_for_lstm_dnn, batch_size=256,
                                                     shuffle=False)
    ...
    residuals_test_loader_for_lstm_dnn = DataLoader(residuals_testing_dataset_for_lstm_dnn, batch_size=256,
                                                    shuffle=False)
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            predictions_before_lstm_after_dnn = torch.cat(
                (predictions_before_lstm_after_dnn, predictions + residuals_dnn), dim=0)
            lengths_brts = torch.sum(data.brts != 0, dim=1).cpu().tolist()
            brts_cpu = data.brts.cpu()
            brts_cpu = brts_cpu.unsqueeze(-1)
            packed_brts = pack_padded_sequence(brts_cpu, lengths_brts, batch_first=True, enforce_sorted=False).to(
            ...
            predictions_after_lstm_after_dnn = torch.cat(
                (predictions_after_lstm_after_dnn, predictions + residuals_dnn + residuals_lstm), dim=0)
    ...
    # compute column-wise mean of residuals
    print("This is using model_lstm and model_dnn to compensate the GNN model")
    ...
    lstm_compensation_data_dict_after_dnn1 = {
        "res_lambda1_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 0],
        "res_lambda2_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 1],
        "res_lambda3_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 2],
        "res_mu1_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 3],
        "res_mu2_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 4],
        "res_lambda1_before_lstm_after_dnn": residuals_before_lstm_after_dnn[:, 0],
        "res_lambda2_before_lstm_after_dnn": residuals_before_lstm_after_dnn[:, 1],
        "res_lambda3_before_lstm_after_dnn": residuals_before_lstm_after_dnn[:, 2],
        "res_mu1_before_lstm_after_dnn": residuals_before_lstm_after_dnn[:, 3],
        "res_mu2_before_lstm_after_dnn": residuals_before_lstm_after_dnn[:, 4],
        "res_lambda1_after_lstm_after_dnn": residuals_after_lstm_after_dnn[:, 0],
        "res_lambda2_after_lstm_after_dnn": residuals_after_lstm_after_dnn[:, 1],
        "res_lambda3_after_lstm_after_dnn": residuals_after_lstm_after_dnn[:, 2],
        "res_mu1_after_lstm_after_dnn": residuals_after_lstm_after_dnn[:, 3],
        "res_mu2_after_lstm_after_dnn": residuals_after_lstm_after_dnn[:, 4],
        "pred_res_lambda1_dnn": predicted_residuals_dnn[:, 0],
        "pred_res_lambda2_dnn": predicted_residuals_dnn[:, 1],
        "pred_res_lambda3_dnn": predicted_residuals_dnn[:, 2],
        "pred_res_mu1_dnn": predicted_residuals_dnn[:, 3],
        "pred_res_mu2_dnn": predicted_residuals_dnn[:, 4],
        "pred_res_lambda1_lstm_after_dnn": predicted_residuals_lstm_after_dnn[:, 0],
        "pred_res_lambda2_lstm_after_dnn": predicted_residuals_lstm_after_dnn[:, 1],
        "pred_res_lambda3_lstm_after_dnn": predicted_residuals_lstm_after_dnn[:, 2],
        "pred_res_mu1_lstm_after_dnn": predicted_residuals_lstm_after_dnn[:, 3],
        "pred_res_mu2_lstm_after_dnn": predicted_residuals_lstm_after_dnn[:, 4],
        "pred_lambda1_before_lstm_before_dnn": predictions_before_lstm_before_dnn[:, 0],
        "pred_lambda2_before_lstm_before_dnn": predictions_before_lstm_before_dnn[:, 1],
        "pred_lambda3_before_lstm_before_dnn": predictions_before_lstm_before_dnn[:, 2],
        "pred_mu1_before_lstm_before_dnn": predictions_before_lstm_before_dnn[:, 3],
        "pred_mu2_before_lstm_before_dnn": predictions_before_lstm_before_dnn[:, 4],
        "pred_lambda1_before_lstm_after_dnn": predictions_before_lstm_after_dnn[:, 0],
        "pred_lambda2_before_lstm_after_dnn": predictions_before_lstm_after_dnn[:, 1],
        "pred_lambda3_before_lstm_after_dnn": predictions_before_lstm_after_dnn[:, 2],
        "pred_mu1_before_lstm_after_dnn": predictions_before_lstm_after_dnn[:, 3],
        "pred_mu2_before_lstm_after_dnn": predictions_before_lstm_after_dnn[:, 4],
        "pred_lambda1_after_lstm_after_dnn": predictions_after_lstm_after_dnn[:, 0],
        "pred_lambda2_after_lstm_after_dnn": predictions_after_lstm_after_dnn[:, 1],
        "pred_lambda3_after_lstm_after_dnn": predictions_after_lstm_after_dnn[:, 2],
        "pred_mu1_after_lstm_after_dnn": predictions_after_lstm_after_dnn[:, 3],
        "pred_mu2_after_lstm_after_dnn": predictions_after_lstm_after_dnn[:, 4],
        "lambda1": y_original[:, 0],
        "lambda2": y_original[:, 1],
        "lambda3": y_original[:, 2],
        "mu1": y_original[:, 3],
    ...
    lstm_compensation_data_df_after_dnn1 = pd.DataFrame(lstm_compensation_data_dict_after_dnn1)
    ...
    # Workaround to get rid of the dtype incompatible issue
    lstm_compensation_data_df_after_dnn1 = lstm_compensation_data_df_after_dnn1.astype(object)

    pyreadr.write_rds(
        os.path.join(name, task_type, "STBO", f"{task_type}_gnn_{gnn_depth}_lstm_dnn_compensation.rds"),
        lstm_compensation_data_df_after_dnn1)
    ...
    # compute column-wise mean of residuals
    print("This is using model_lstm_dnn and model_dnn to compensate the GNN model")
    print("Mean residuals before LSTM compensation before DNN compensation:")
    print(np.mean(abs(residuals_before_lstm_before_dnn), axis=0))
    print("Mean residuals before LSTM compensation after DNN compensation:")
    print(np.mean(abs(residuals_before_lstm_after_dnn), axis=0))
    ...
    lstm_compensation_data_dict_after_dnn = {
        "res_lambda1_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 0],
        "res_lambda2_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 1],
        "res_lambda3_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 2],
        "res_mu1_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 3],
    ...
    residuals_train_loader_for_dnn_lstm = DataLoader(residuals_training_dataset_for_dnn_lstm, batch_size=256,
                                                     shuffle=False)
    ...
    residuals_test_loader_for_dnn_lstm = DataLoader(residuals_testing_dataset_for_dnn_lstm, batch_size=256,
                                                    shuffle=False)
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            predictions_before_dnn_after_lstm = torch.cat(
                (predictions_before_dnn_after_lstm, predictions + residuals_lstm), dim=0)
            ...
            predictions_after_dnn_after_lstm = torch.cat(
                (predictions_after_dnn_after_lstm, predictions + residuals_lstm + residuals_dnn), dim=0)
    ...
    dnn_compensation_data_dict_after_lstm = {
        "res_lambda1_before_dnn_before_lstm": residuals_before_dnn_before_lstm[:, 0],
        "res_lambda2_before_dnn_before_lstm": residuals_before_dnn_before_lstm[:, 1],
        "res_lambda3_before_dnn_before_lstm": residuals_before_dnn_before_lstm[:, 2],
        "res_mu1_before_dnn_before_lstm": residuals_before_dnn_before_lstm[:, 3],
        "res_mu2_before_dnn_before_lstm": residuals_before_dnn_before_lstm[:, 4],
        "res_lambda1_before_dnn_after_lstm": residuals_before_dnn_after_lstm[:, 0],
        "res_lambda2_before_dnn_after_lstm": residuals_before_dnn_after_lstm[:, 1],
        "res_lambda3_before_dnn_after_lstm": residuals_before_dnn_after_lstm[:, 2],
        "res_mu1_before_dnn_after_lstm": residuals_before_dnn_after_lstm[:, 3],
        "res_mu2_before_dnn_after_lstm": residuals_before_dnn_after_lstm[:, 4],
        "res_lambda1_after_dnn_after_lstm": residuals_after_dnn_after_lstm[:, 0],
        "res_lambda2_after_dnn_after_lstm": residuals_after_dnn_after_lstm[:, 1],
        "res_lambda3_after_dnn_after_lstm": residuals_after_dnn_after_lstm[:, 2],
        "res_mu1_after_dnn_after_lstm": residuals_after_dnn_after_lstm[:, 3],
        "res_mu2_after_dnn_after_lstm": residuals_after_dnn_after_lstm[:, 4],
        "pred_res_lambda1_lstm": predicted_residuals_lstm[:, 0],
        "pred_res_lambda2_lstm": predicted_residuals_lstm[:, 1],
        "pred_res_lambda3_lstm": predicted_residuals_lstm[:, 2],
        "pred_res_mu1_lstm": predicted_residuals_lstm[:, 3],
        "pred_res_mu2_lstm": predicted_residuals_lstm[:, 4],
        "pred_res_lambda1_dnn_after_lstm": predicted_residuals_dnn_after_lstm[:, 0],
        "pred_res_lambda2_dnn_after_lstm": predicted_residuals_dnn_after_lstm[:, 1],
        "pred_res_lambda3_dnn_after_lstm": predicted_residuals_dnn_after_lstm[:, 2],
        "pred_res_mu1_dnn_after_lstm": predicted_residuals_dnn_after_lstm[:, 3],
        "pred_res_mu2_dnn_after_lstm": predicted_residuals_dnn_after_lstm[:, 4],
        "pred_lambda1_before_dnn_before_lstm": predictions_before_dnn_before_lstm[:, 0],
        "pred_lambda2_before_dnn_before_lstm": predictions_before_dnn_before_lstm[:, 1],
        "pred_lambda3_before_dnn_before_lstm": predictions_before_dnn_before_lstm[:, 2],
        "pred_mu1_before_dnn_before_lstm": predictions_before_dnn_before_lstm[:, 3],
        "pred_mu2_before_dnn_before_lstm": predictions_before_dnn_before_lstm[:, 4],
        "pred_lambda1_before_dnn_after_lstm": predictions_before_dnn_after_lstm[:, 0],
        "pred_lambda2_before_dnn_after_lstm": predictions_before_dnn_after_lstm[:, 1],
        "pred_lambda3_before_dnn_after_lstm": predictions_before_dnn_after_lstm[:, 2],
        "pred_mu1_before_dnn_after_lstm": predictions_before_dnn_after_lstm[:, 3],
        "pred_mu2_before_dnn_after_lstm": predictions_before_dnn_after_lstm[:, 4],
        "pred_lambda1_after_dnn_after_lstm": predictions_after_dnn_after_lstm[:, 0],
        "pred_lambda2_after_dnn_after_lstm": predictions_after_dnn_after_lstm[:, 1],
        "pred_lambda3_after_dnn_after_lstm": predictions_after_dnn_after_lstm[:, 2],
        "pred_mu1_after_dnn_after_lstm": predictions_after_dnn_after_lstm[:, 3],
        "pred_mu2_after_dnn_after_lstm": predictions_after_dnn_after_lstm[:, 4],
        "lambda1": y_original[:, 0],
        "lambda2": y_original[:, 1],
        "lambda3": y_original[:, 2],
        "mu1": y_original[:, 3],
