...
with open("../Config/ddd_train_cnn1d.yaml", "r") as ymlfile:
    global_params = yaml.safe_load(ymlfile)

# Set global variables
cap_norm_factor = global_params["cap_norm_factor"]
epoch_number_cnn1d = global_params["epoch_number_cnn1d"]
...
def main():
    ...
    current_dataset = read_rds_to_pytorch(full_dir, rds_count, False)
    # Shuffle the data
    ...
    current_val_dataset = read_rds_to_pytorch(val_dir, val_rds_count, False)
    ...
    class CNNModel(torch.nn.Module):
        def __init__(self):
            ...
            # Convolution 1: (in=1, out=8) -> output size: 8 x 1991
            self.conv1 = torch.nn.Conv1d(in_channels=1, out_channels=8, kernel_size=10, stride=1)
            # Pooling 1 (average): kernel_size=2, stride=2 -> 8 x 995

            # Convolution 2: (in=8, out=16) -> output size: 16 x 986
            self.conv2 = torch.nn.Conv1d(in_channels=8, out_channels=16, kernel_size=10, stride=1)
            # Pooling 2 (average): kernel_size=2, stride=2 -> 16 x 493

            # Convolution 3: (in=16, out=32) -> output size: 32 x 484
            self.conv3 = torch.nn.Conv1d(in_channels=16, out_channels=32, kernel_size=10, stride=1)
            # Pooling 3 (average): kernel_size=2, stride=2 -> 32 x 242

            # Convolution 4: (in=32, out=64) -> output size: 64 x 233
    ...
    def compute_test_loss_cnn1d():
        ...
        return loss_all / len(test_loader.dataset)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Training using {device}")

    model_cnn1d = CNNModel()
    ...
    for epoch in range(1, epoch_number_cnn1d):
        actual_epoch_cnn1d = epoch
        train_loss_all = train_cnn1d()
        test_loss_all = compute_test_loss_cnn1d()
        test_loss_all = test_loss_all * train_test_ratio
        test_mean_diffs, test_diffs_all, test_predictions, test_y, test_nodes_all = test_diff_cnn1d(test_loader)
    ...
    data_dict["Epoch"] = list(range(1, actual_epoch_cnn1d + 1))
