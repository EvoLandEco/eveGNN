...
def main():
    ...
    print(f'Name: {name}, Task Type: DDD Validation', f'GNN Depth: {gnn_depth}')
    print("Now on branch Multimodal-Stacking-Boosting")

    validation_dataset_list = []
    val_dir = os.path.join(name, "DDD_FREE_TES_VAL")
    ...
    if not os.path.exists(os.path.join(name, "DDD_FREE_TES", "VAL")):
        os.makedirs(os.path.join(name, "DDD_FREE_TES", "VAL"))

    # Save the data to a file using pyreadr
    pyreadr.write_rds(os.path.join(name, "DDD_FREE_TES", "VAL", f"DDD_FREE_TES_VAL_final_diffs_diffpool_{gnn_depth}.rds"),
                      final_differences)
    pyreadr.write_rds(os.path.join(name, "DDD_FREE_TES", "VAL", f"DDD_FREE_TES_VAL_final_predictions_diffpool_{gnn_depth}.rds"),
                      final_predictions)
