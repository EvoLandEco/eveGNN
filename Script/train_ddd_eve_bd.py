...
# Paths to the tables
bd_params_path = "BD_TAS/BD_TAS_params.txt"
bd_tree_path = "BD_TAS/set_1"
ddd_params_path = "DDD_TAS/DDD_TAS_params.txt"
ddd_tree_path = "DDD_TAS"
eve_params_path = "EVE_TAS/EVE_TAS_params.txt"
eve_tree_path = "EVE_TAS"
...
def main():
    ...
    for model_name in ['DDD_TAS', 'EVE_TAS']:
        for set_path in list_subdirectories(ddd_tree_path):
            ...
            full_dir = os.path.join(model_name, set_path)
            ...
            params_current = get_params(model_name, set_path)
            ...
            print(f'There are: {rds_count} trees in the {set_path} folder.')
            print(f"Now reading {model_name}:{set_path}...")
            # Read the .rds files into a list of PyTorch Geometric Data objects
            current_path = os.path.join(model_name, set_path)
            current_dataset = read_rds_to_pytorch(current_path, model_name, rds_count)
            ...
            pyreadr.write_rds(os.path.join(model_name, f"{write_data_name}.rds"), model_performance)
