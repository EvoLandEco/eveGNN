...
def main():
    ...
    for epoch in range(1, epoch_number_gnn):
        ...
        print(
            f'Epoch: {epoch:03d}, Par 1 Mean Diff: {test_mean_diffs[0]:.4f}, Par 2 Mean Diff: {test_mean_diffs[1]:.4f}, Train Loss: {train_loss_all:.4f}, Test Loss: {test_loss_all:.4f}')
    ...
    final_differences = pd.DataFrame(final_test_diffs, columns=["lambda_diff", "mu_diff"])
    final_predictions = pd.DataFrame(final_test_predictions, columns=["lambda_pred", "mu_pred"])
    final_y = pd.DataFrame(final_test_y, columns=["lambda", "mu"])
    final_differences["num_nodes"] = final_test_nodes

    print("Final differences:")
    print(abs(final_differences[["lambda_diff", "mu_diff"]]).mean())
    ...
    # Use both the trained LSTM and DNN to compensate the GNN model, and test the compensated model
    ...
    lstm_compensation_data_dict_after_dnn1 = {"res_lambda_before_lstm_before_dnn": residuals_before_lstm_before_dnn[:, 0],
