...
def main():
    ...
    def boost_bt_val(scenario, partite, model):
        ...
        model_path_gnn = os.path.join(name, task_type, "STBO", f"{task_type}_model_diffpool_{gnn_depth}_gnn_{model}.pt")
        model_gnn.load_state_dict(torch.load(model_path_gnn))
        ...
        print(f"Now evaluating GNN model using pre-trained weights on {model}...")
        ...
        pyreadr.write_rds(os.path.join(name, task_type, "STBO", f"{task_type}_final_diffpool_{gnn_depth}_gnn_pretrained_{model}.rds"),
                          final_result)

        # Use pre-trained LSTM model to compensate the GNN model, and test the compensated model
        ...
        model_path_lstm = os.path.join(name, task_type, "STBO", f"{task_type}_model_diffpool_{gnn_depth}_lstm_{model}.pt")
        ...
        pyreadr.write_rds(os.path.join(name, task_type, "STBO", f"{task_type}_gnn_{gnn_depth}_lstm_compensation_pretrained_{model}.rds"),
                          lstm_compensation_data_df)

        print(f"Training and testing completed for scenario: {scenario} {partite} on pre-trained model: {model}")

    # Execute the Boost BT function for each scenario
    boost_bt_val(scenario_param, partite_param, "pd")
    boost_bt_val(scenario_param, partite_param, "ed")
    boost_bt_val(scenario_param, partite_param, "nnd")
