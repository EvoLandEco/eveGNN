...
with open("../Config/pbd_val_diffpool.yaml", "r") as ymlfile:
...
def main():
    ...
    if task_type == "PBD_VAL_TES":
        full_dir = os.path.join(name, "PBD_FREE_TES")
        val_dir = os.path.join(name, "PBD_VAL_TES")
        model_type = "PBD_FREE_TES"
        val_batch_size_adjusted = int(val_batch_size)
    elif task_type == "PBD_VAL_TAS":
        full_dir = os.path.join(name, "PBD_FREE_TAS")
        val_dir = os.path.join(name, "PBD_VAL_TAS")
        model_type = "PBD_FREE_TAS"
    ...
    for i in range(1, max_gnn_depth + 1):
        ...
        print(f'Par 1 Mean Diff: {test_mean_diffs[0]:.4f}, Par 2 Mean Diff: {test_mean_diffs[1]:.4f}, Par 3 Mean Diff: {test_mean_diffs[2]:.4f}, Par 4 Mean Diff: {test_mean_diffs[3]:.4f}, Par 5 Mean Diff: {test_mean_diffs[4]:.4f}')
        print(f"Final test diffs length: {len(test_diffs_all)}")

        # Convert the dictionary to a pandas DataFrame
        final_differences = pd.DataFrame(test_diffs_all, columns=["lambda1_diff", "lambda2_diff", "lambda3_diff", "mu1_diff", "mu2_diff"])
        final_predictions = pd.DataFrame(test_predictions, columns=["lambda1_pred", "lambda2_pred", "lambda3_pred", "mu1_pred", "mu2_pred"])
        final_y = pd.DataFrame(test_y, columns=["lambda1", "lambda2", "lambda3", "mu1", "mu2"])
